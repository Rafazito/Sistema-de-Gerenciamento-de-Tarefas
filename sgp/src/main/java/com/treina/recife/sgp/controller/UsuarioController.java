...
@GetMapping
public ResponseEntity<Page<Usuario>> getUsuarios(@PageableDefault(sort = "usuarioId", direction = Sort.Direction.ASC) Pageable pageable){
...
public ResponseEntity<Object> atualizarStatus(@PathVariable(value = "userId") long userId, @RequestBody Map<String, String> body) {
    ...
    try {
        ...
        return ResponseEntity.badRequest().body("Status inválido Valores permitidos: ATIVO e INATIVO");
    }
    
    Usuario usuarioAUsuario = usuario.get();
    usuarioAUsuario.setStatus(novoStatus);

    usuarioService.updateUsuario(usuarioAUsuario);
    logger.info("Status do usuário atualizado com sucesso.");

    return ResponseEntity.status(HttpStatus.OK).body(usuarioAUsuario);
